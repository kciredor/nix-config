  ...
  home-manager = {
    ...
    users.kciredor = { pkgs, lib, ... }: {
      home.packages = with pkgs; [
        coreutils
        (hiPrio binutils)  # Priority over gcc (collision: both provide ld). Required by gdb-gef.
        diffutils
        findutils
        gnused
        gnutar
      ...
      programs.fish = {
        ...
        shellInit = ''
          ...
          set -xg EDITOR nvim
          set -xg PATH "${config.users.users.kciredor.home}/bin:$PATH"
        '';
      ...
      programs.gpg = {
        ...
        publicKeys = [
          {
            text = lib.strings.fileContents "${config.users.users.kciredor.home}/ops/nix-config/includes/kciredor/gpg_pubkey";
            trust = 5;
          }
        ];
      };
      ...
      programs.ssh = {
        ...
        includes = [
          "${config.users.users.kciredor.home}/ops/nix-config/secrets/kciredor/ssh_hosts"
        ];
      };
      ...
      programs.neovim = {
        ...
        extraConfig = builtins.concatStringsSep "\n" [
          (lib.strings.fileContents "${config.users.users.kciredor.home}/ops/nix-config/dotfiles/kciredor/neovim/base.vim")
          (lib.strings.fileContents "${config.users.users.kciredor.home}/ops/nix-config/dotfiles/kciredor/neovim/plugins.vim") 

          ''
            lua << EOF
            ${lib.strings.fileContents "${config.users.users.kciredor.home}/ops/nix-config/dotfiles/kciredor/neovim/plugins.lua"}
            ${lib.strings.fileContents "${config.users.users.kciredor.home}/ops/nix-config/dotfiles/kciredor/neovim/lsp.lua"}
            EOF
          ''
        ];
